...
class TestPlannerAgent:
    ...
    Per ``mock.patch`` decorator stacking rules (bottom-up mock-arg
    convention), the decorator order below maps to signature:
        ``(self, mock_helper_invoke, mock_invoke, mock_trace)``
    """

    @pytest.mark.asyncio
    @patch("agents.planner.trace_agent")
    @patch("agents.planner.invoke_messages")
    @patch("agents._planner_helpers.invoke_messages")
    async def test_plan_creates_research_plan(
        self, mock_helper_invoke: AsyncMock, mock_invoke: AsyncMock, mock_trace: MagicMock,
    ) -> None:
    ...
    async def test_adversarial_challenge_fails_plan_refined(
        ...
        # Each mock has its own side_effect list — separate iterators.
