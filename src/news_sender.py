...
from logging_config import setup_terminal_logging, separator
...
async def run_news_sender(theme_path: Path | None = None) -> dict[str, bool]:
    ...
    if not channels and not has_file_output:
        logger.warning("No channels or file output enabled — set NEWS_SEND_EMAIL / NEWS_SEND_TELEGRAM / NEWS_SEND_DISCORD / NEWS_OUTPUT_FILE")
        return {}
    if channels:
    ...
    try:
        ...
        # 4. Build report
        report_body = result.final_report.summary if result.final_report else "(no report produced)"
        subject = _build_subject(theme, result.judge_score)
        logger.info("Report ready | chars=%s score=%s", len(report_body), result.judge_score)
        ...
        if output_file:
            try:
                ...
                logger.info("Report written to %s (%d bytes)", out_path.resolve(), out_path.stat().st_size)
            except Exception as exc:
