...
_GAP_LOG_PATH = Path("data_gaps.jsonl")  # T18: machine-readable gap export
# ────────────────────────────────────────────────────────────────────
...
def _export_data_gaps(
    ...
    """T18: Persist uncovered missing_topics as machine-readable JSONL.
...
def _check_source_diversity(
    ...
    """T19: Return a warning string if sources collapse to a single domain.
...
def _validate_summary_has_query_signals(
    summary: str,
    query: str,
) -> bool:
...
def _empty_report() -> ResearchReport:
    """Trivial report for no-evidence / low-context scenarios."""
    ...
    with trace_agent("summarizer", input_data={"query": state.query}) as tracer:
        ...
        if needs_retry:
            ...
            stricter_system = system_content + (
                ...
                + (f"- Summary must contain query-related keywords or [N] citation markers\n"
