...
async def fact_check(state: ResearchState) -> ResearchState:
    ...
    with trace_agent(
        ...
        for item in raw_evidence:
            if await memory._is_semantic_duplicate(item.fact, threshold=current_threshold):
                logger.debug(
        ...
        _REGULATORY_TERMS = frozenset([
            "regulation", "directive", "act", "guideline", "guidance",
            "fda", "ema", "who", "law", "legislation", "compliance",
            "official", "standard", "framework", "policy",
        ])
